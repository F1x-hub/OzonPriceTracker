...
- `.agents/rules/agent.md`: изменения только внутри `MarketPilot`, MV3, существующие владельцы popup/background/content.
- `MarketPilot/popup.html`: текущая AI-вкладка и поле промпта вопросов.
- `MarketPilot/popup.js`: текущая загрузка/автосохранение AI-настроек и prompt chips.
- `MarketPilot/background.js`: текущий DeepSeek gateway и общий учёт статистики.
- `MarketPilot/seller_content.js`: текущий выбор шаблонов отзывов, отправка отзывов и аналогичный AI-режим вопросов.
...
- Modify `MarketPilot/popup.html`: добавить отдельную textarea промпта отзывов, reset и chips `{product}`, `{review}`, `{date}`.
- Modify `MarketPilot/popup.js`: объявить дефолт, загрузку, autosave, reset и сохранение `deepseekReviewPrompt`.
- Modify `MarketPilot/background.js`: добавить message action `deepseek_generate_review_answer` и handler с заменой плейсхолдеров.
- Modify `MarketPilot/seller_content.js`: добавить AI option в review dropdowns и вызвать новый handler перед заполнением формы.
...
1. `node --check MarketPilot/background.js`, `node --check MarketPilot/popup.js`, `node --check MarketPilot/seller_content.js`.
2. Parse `MarketPilot/manifest.json` and ensure the existing DeepSeek host permission remains present.
